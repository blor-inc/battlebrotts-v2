...
var velocity: Vector2 = Vector2.ZERO  # S17.2-003: true velocity vector — source of truth for smoothed-intent position updates
var current_speed: float = 0.0  # current movement speed (px/s) — target magnitude for velocity smoothing
...
# S17.2-003: Reversal-damping timer. When forward-intent reversal >=120° occurs,
# armed to REVERSAL_DAMPING_TICKS; magnitude target is scaled by
# REVERSAL_DAMPING_FACTOR while >0. Consumed in _smooth_velocity.
...
func setup() -> void:
	...
	# S17.2-003: Per-chassis angular-velocity cap. Values from
	# docs/design/s17.2-scout-feel.md §4.1. Kept here (not in chassis_data.gd)
	# to preserve the data/** scope gate.
	match chassis_type:
		ChassisData.ChassisType.SCOUT:
			max_angular_velocity = deg_to_rad(540.0)  # 1.5 turns/s — nimble, not magical
